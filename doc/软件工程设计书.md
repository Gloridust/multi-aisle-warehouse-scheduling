...
系统建模的仓库为单巷道、单堆垛机的高空立体仓库，其物理结构可以抽象为一个二维网格：

- **横轴（列）**：堆垛机沿巷道水平移动方向，共 N 列
- **纵轴（层）**：堆垛机垂直提升方向，共 M 层
- **货位**：网格中的每一个格子即一个货位，对应一个托盘位
- **入库口**：固定在第1层第1列的位置
- **堆垛机**：单台，每次运送一个托盘，从入库口出发到目标货位，再返回入库口
...
```
T(i,j) = max( |col - entry_col| × horizontal_speed, |row - entry_row| × vertical_speed )
```

其中 `(row, col)` 是目标货位坐标，`(entry_row, entry_col)` 是入库口坐标。每次入库作业堆垛机需往返一趟，总耗时为 `2 × T(i,j)`。
...
**货位管理**：查看所有货位状态，支持按层筛选，手动锁定/解锁货位。
...
页面分为上下两部分。上半部分是三个并排的仓库可视化网格图，分别对应三种策略的分配结果。每个网格图是一个 M行×N列 的二维矩阵，每个格子代表一个货位：

- 灰色 = 空闲
- 不同颜色按商品分类着色（如手机=蓝色，电脑=绿色，冰箱=橙色）
- 鼠标悬浮货位时，tooltip 显示：货位坐标、商品名称、存放件数、空间占用率、存取距离
...
独立的仓库实时状态页面，展示当前仓库各货位的真实占用情况。支持按层切换显示（如果仓库很大可以只看某一层），鼠标悬浮查看货位详情。与仿真模块的区别是：这里展示的是真实数据，仿真模块展示的是模拟数据。
...
GET    /api/warehouse/{id}/locations?row=1  获取货位列表（支持按层筛选）
```
